...
/*:
 * @target MZ
 * @plugindesc Nothing depends on this, it's just random stuff that didn't fit anywhere else.
 ...
 * @text Control
 * 
    * @param overrideKeybindings
    * @text Override Keybindings
    * @parent CAT_CONTROL
    * @desc Enable key bindings override.
    * @type select
    * @option ON
    * @option OFF
    * @default OFF
    ...
    * @text Keybindings
    * @parent CAT_CONTROL
    * @desc Customize control bindings here.
    * @type struct<KeyBind>[]
    * @default ["{\"Name\":\"down\",\"Key\":\"40\"}","{\"Name\":\"down\",\"Key\":\"98\"}","{\"Name\":\"down\",\"Key\":\"83\"}","{\"Name\":\"left\",\"Key\":\"37\"}","{\"Name\":\"left\",\"Key\":\"100\"}","{\"Name\":\"left\",\"Key\":\"65\"}","{\"Name\":\"right\",\"Key\":\"39\"}","{\"Name\":\"right\",\"Key\":\"102\"}","{\"Name\":\"right\",\"Key\":\"68\"}","{\"Name\":\"up\",\"Key\":\"38\"}","{\"Name\":\"up\",\"Key\":\"104\"}","{\"Name\":\"up\",\"Key\":\"87\"}","{\"Name\":\"ok\",\"Key\":\"13\"}","{\"Name\":\"ok\",\"Key\":\"32\"}","{\"Name\":\"ok\",\"Key\":\"90\"}","{\"Name\":\"cancel\",\"Key\":\"88\"}","{\"Name\":\"escape\",\"Key\":\"45\"}","{\"Name\":\"escape\",\"Key\":\"96\"}","{\"Name\":\"escape\",\"Key\":\"27\"}","{\"Name\":\"shift\",\"Key\":\"16\"}","{\"Name\":\"tab\",\"Key\":\"9\"}","{\"Name\":\"pageup\",\"Key\":\"33\"}","{\"Name\":\"pagedown\",\"Key\":\"34\"}","{\"Name\":\"control\",\"Key\":\"17\"}","{\"Name\":\"control\",\"Key\":\"18\"}","{\"Name\":\"debug\",\"Key\":\"120\"}"]
    * 
    * @param buttonBinds
    * @text Button binds
    * @parent CAT_CONTROL
    * @desc Customize gamepad bindings here.
    * @type struct<ButtonBind>[]
    * @default ["{\"Name\":\"ok\",\"Button\":\"0\"}","{\"Name\":\"cancel\",\"Button\":\"1\"}","{\"Name\":\"shift\",\"Button\":\"2\"}","{\"Name\":\"menu\",\"Button\":\"3\"}","{\"Name\":\"pageup\",\"Button\":\"4\"}","{\"Name\":\"pagedown\",\"Button\":\"5\"}","{\"Name\":\"up\",\"Button\":\"12\"}","{\"Name\":\"down\",\"Button\":\"13\"}","{\"Name\":\"left\",\"Button\":\"14\"}","{\"Name\":\"right\",\"Button\":\"15\"}"]
 * 
 * @param CAT_TEXT
 * @text Text
 * 
 ...
 * @text Window
 * 
    * @param messageHorizontalMargin
    * @text Message Horizontal Margin
    * @parent CAT_WINDOW
    * @type number
    * @default 0
    * 
    * @param messageVerticalMargin
    * @text Message Vertical Margin
    * @parent CAT_WINDOW
    * @type number
    * @default 0
    * 
    * @param windowItemColorTop
    * @text Window Item Color Top
    * @parent CAT_WINDOW
    * @default rgba(32, 32, 32, 0.5)
    * 
    * @param windowItemColorBottom
    * @text Window Item Color Bottom
    * @parent CAT_WINDOW
 ...
 * 
    ...
    * @text Disable Smooth
    * @parent CAT_CONFIGS
    * @desc Disables the blurry look of MZ's rendering.
    * @type boolean
    * @default true
    * 
    * @param Custom formulas
    * @text Custom formulas
    * @parent CAT_CONFIGS
    * @desc Custom formula shorthands for skills/items/etc.
    * @type struct<Formula>[]
    * @default []
    * 
    * @param minDamageIsOne
    * @text Make min damage one
    * @parent CAT_CONFIGS
    * @desc Low-number games can be frustrating to balance if everything does 0dmg.
    * @type boolean
    * @default false
    * 
    * @param CAT_SCALE
    ...
    * @parent CAT_CONFIGS
    * 
        * @param addScaleToOptions
        * @text Add Scale to Options
        * @parent CAT_SCALE
        * @type boolean
        * @default true
        * 
        * @param scaleDisplayName
        * @text Scale display name
        * @parent CAT_SCALE
    ...
    * @parent CAT_CONFIGS
    * 
        * @param addFilterToOptions
        * @text Add Filter to Options
        * @parent CAT_FILTER
        * @type boolean
        * @default true
        * 
        * @param filterDisplayName
        * @text Filter display name
        * @parent CAT_FILTER
        * @default CRT Filter
        * 
        * @param defaultFilter
...
(() => {
    /* Helper functions */
    const PLUGIN_NAME = "OZZ_Metal";
    const SETSCALE_SYM = "scale";
    const SETFILTER_SYM = "filter";

//#region Embedded pixi-filters.js
...
//#region Helper class definition
    class OZMetal {
        constructor() {
            ...
            this.overrideKeyInputBind = params['overrideKeybindings'] === "ON";
            this.disableSmooth = params['disableSmooth']==='true';
            this.minDamageOne = params['minDamageIsOne']==='true';
            ...
            for (var i = 0; i < this.keybinds.length; i++) {
                this.keybinds[i] = JSON.parse(this.keybinds[i]);
                this.keybinds[i].Key = Number(this.keybinds[i].Key);
            }
            this.gamepadBinds = JSON.parse(params['buttonBinds']);
            for (var i = 0; i < this.gamepadBinds.length; i++) {
                this.gamepadBinds[i] = JSON.parse(this.gamepadBinds[i]);
                this.gamepadBinds[i].Button = Number(this.gamepadBinds[i].Button);
            }
            this.text = {};
            this.text.outlineColor = params['textOutlineColor'];
            this.text.itemBackColor1 = params['windowItemColorTop'];
            this.text.itemBackColor2 = params['windowItemColorBottom'];
            this.message = {};
            this.message.marginHorizontal = Number(params['messageHorizontalMargin']);
            this.message.marginVertical = Number(params['messageVerticalMargin']);
            Game_BattlerBase.ICON_BUFF_START = 64;
            Game_BattlerBase.ICON_DEBUFF_START = 80;
            this.terms = {};
            this.terms.scale = params['scaleDisplayName'];
            this.terms.filter = params['filterDisplayName'];
            this.configs = {};
            this.configs.defaultScale = Number(params['defaultScale']);
            this.configs.maxScale = Number(params['maxScale']);
            this.configs.addScaleToConfig = params['addScaleToOptions'] == "true";
            this.configs.defaultFilter = params['defaultFilter'] == "true";
            this.configs.addFilterToConfig = params['addFilterToOptions'] == "true";
            this._currentFilter = this.configs.defaultFilter;
            this._currentScale = this.configs.defaultScale;
            this._filterTimer = 0;
        }
        CheckMaxScale() {
            let target = this.configs.maxScale;
            const mH = (window.outerWidth - window.innerWidth);
            const mV = (window.outerHeight - window.innerHeight);
            const maxSH = Math.floor((screen.availWidth - mH) / $dataSystem.advanced.screenWidth);
            const maxSV = Math.floor((screen.availHeight - mV) / $dataSystem.advanced.screenHeight);
            let maxScale = Math.max(Math.min(maxSH, maxSV, target), 1);
            this.configs.maxScale = maxScale;
        }
        GetScale() {
            return this._currentScale;
        }
        SetDefaultScale() {
            this.ResizeWindow(this.configs.defaultScale);
        }
        ResizeWindow(factor) {
            let marginHorz = (window.outerWidth - window.innerWidth);
            let marginVert = (window.outerHeight - window.innerHeight);
            if (factor < 1) factor = 1;
            if (factor > this.configs.maxScale) factor = this.configs.maxScale;
            this._currentScale = factor;
            let w = $dataSystem.advanced.screenWidth; // > 0 ? Graphics.width : window.innerWidth;
            let h = $dataSystem.advanced.screenHeight; // > 0 ? Graphics.height : window.innerHeight;
            window.resizeTo(marginHorz + w * factor, marginVert + h * factor);
            this.Center();
        }
        Center() {
            let x = (screen.availWidth - window.outerWidth) / 2;
            let y = (screen.availHeight - window.outerHeight) / 2;
            window.moveTo(x,y);
        }
        GetFilter() {
        ...
        UpdateFilter(scene) {
            this._filterTimer += 0.05;
            if (this._currentFilter) {
                let f = scene.filters;
                let idx = -1;
                for (var i=0; i < f.length; i++) {
                    if (f[i] instanceof PIXI.filters.CRTFilter) idx = i;
                }
                if (idx != -1) {
                    f[idx].time = this._filterTimer;
                }
            }
        }
        RefreshFilter(scene) {
            if (scene == null) {
                if (SceneManager._scene == null) return;
            ...
            if (this._currentFilter) {
                if (idx == -1) {
                    let options = {};
                    let crt = new PIXI.filters.CRTFilter(options);
                    f.push(crt);
                }
            } else {
                if (idx != -1) {
        ...
        Formula(name,a,b,v) {
            const formula = function(name) {
                return OZ.core.Formula(name,a,b,v);
            }
            let f = this.formulas[name];
            if (typeof f === 'undefined') return 0;
            return eval(f.Contents);
        }
    }
//#endregion
...
//#region Damage formula stuff
    Game_Action.prototype.evalDamageFormula = function(target) {
        try {
            ...
            const min = (item.damage.type==0 || !OZ.core.minDamageOne) ? 0 : 1;
            const value = Math.max(eval(item.damage.formula), min) * sign;
...
//#region Disable smooth
// Credits to KisaiTenshi @ https://forums.rpgmakerweb.com/index.php?threads/how-to-remove-blur.47504/
// Inject pixelated rendering.
if (OZ.core.disableSmooth) {
    var styles = "CANVAS{image-rendering: pixelated;}";
    var styleSheet = document.createElement("style");
    styleSheet.innerText = styles;
    document.head.appendChild(styleSheet);
    var OZZM_Bitmap_initialize = Bitmap.prototype.initialize;
    Bitmap.prototype.initialize = function(width, height) {
        OZZM_Bitmap_initialize.call(this,width,height);
        this._smooth = false;
    }
    // Set bitmaps to no-smooth.
    ImageManager.loadBitmap = function(folder, filename) {
        if (filename) {
            const url = folder + Utils.encodeURI(filename) + ".png";
            var _b_ = this.loadBitmapFromUrl(url);
            _b_.smooth = false;
            return _b_;
        } else {
            this._emptyBitmap.smooth = false;
...
Object.defineProperty(ConfigManager, "scale", {
    get: function() {
        return OZ.core.GetScale();
    },
    set: function(value) {
        OZ.core.ResizeWindow(value);
    },
    configurable: true
});
Object.defineProperty(ConfigManager, "filter", {
    get: function() {
        return OZ.core.GetFilter();
    },
    set: function(value) {
        OZ.core.SetFilter(value);
    },
    configurable: true
});

OZZM_ConfigManager_makeData = ConfigManager.makeData;
ConfigManager.makeData = function() {
    const config = OZZM_ConfigManager_makeData.call(this);
    config.scale = this.scale;
    config.filter = this.filter;
    return config;
};

OZZM_ConfigManager_applyData = ConfigManager.applyData;
ConfigManager.applyData = function(config) {
    OZZM_ConfigManager_applyData.call(this,config);
    this.scale = this.readNumeric(config, "scale", OZ.core.configs.defaultScale);
    this.filter = this.readFlag(config, "filter", true);
};

ConfigManager.readNumeric = function(config, name, defVal) {
    if (name in config) {
...
Scene_Boot.prototype.adjustWindow = function() {
    if (Utils.isNwjs()) {
        OZ.core.CheckMaxScale();
        OZ.core.ResizeWindow(OZ.core.configs.addScaleToConfig ? ConfigManager.scale : OZ.core.configs.defaultScale);
    }
};
//#endregion

//#region Filters
OZZPF_Scene_Base_initialize = Scene_Base.prototype.initialize;
Scene_Base.prototype.initialize = function() {
    OZZPF_Scene_Base_initialize.call(this);
    OZ.core.RefreshFilter(this);
};
OZZPF_Scene_Base_update = Scene_Base.prototype.update;
Scene_Base.prototype.update = function() {
    OZ.core.UpdateFilter(this);
    OZZPF_Scene_Base_update.call(this);
};
//#endregion

//#region Option config.
OZZM_Window_Options_makeCommandList = Window_Options.prototype.makeCommandList;
Window_Options.prototype.makeCommandList = function() {
    OZZM_Window_Options_makeCommandList.call(this);
    this.addVideoOptions();
};

Window_Options.prototype.addVideoOptions = function() {
    if (OZ.core.configs.addScaleToConfig) {
        this.addCommand(OZ.core.terms.scale, SETSCALE_SYM);
    }
    if (OZ.core.configs.addFilterToConfig) {
        this.addCommand(OZ.core.terms.filter, SETFILTER_SYM);
    }
};

OZZM_Window_Options_statusText = Window_Options.prototype.statusText;
Window_Options.prototype.statusText = function(index) {
    const symbol = this.commandSymbol(index);
    const value = this.getConfigValue(symbol);
    if (symbol == SETSCALE_SYM) {
        return value + "x";
    }
    return OZZM_Window_Options_statusText.call(this, index);
};

OZZM_Window_Options_processOk = Window_Options.prototype.processOk;
Window_Options.prototype.processOk = function() {
    const index = this.index();
    const symbol = this.commandSymbol(index);
    if (symbol == SETSCALE_SYM) {
        let max = OZ.core.configs.maxScale;
        this.changeNumericValue(symbol, 1, max, true);
    } else if (symbol == SETFILTER_SYM) {
        this.changeValue(symbol, !this.getConfigValue(symbol));
    } else {
        OZZM_Window_Options_processOk.call(this);
    }
};

OZZM_Window_Options_cursorRight = Window_Options.prototype.cursorRight;
Window_Options.prototype.cursorRight = function() {
    const index = this.index();
    const symbol = this.commandSymbol(index);
    if (symbol == SETSCALE_SYM) {
        let max = OZ.core.configs.maxScale;
        this.changeNumericValue(symbol, 1, max, false);
    } else if (symbol == SETFILTER_SYM) {
        this.changeValue(symbol, true);
    } else {
        OZZM_Window_Options_cursorRight.call(this);
    }
};

OZZM_Window_Options_cursorLeft = Window_Options.prototype.cursorLeft;
Window_Options.prototype.cursorLeft = function() {
    ...
    if (symbol == SETSCALE_SYM) {
        ...
        OZZM_Window_Options_cursorLeft.call(this);
    }
};

Window_Options.prototype.changeNumericValue = function(symbol, offset, max, wrap) {
    const lastValue = this.getConfigValue(symbol);
    const value = lastValue + offset;
    if (value > max && wrap) {
        this.changeValue(symbol, 0);
    } else {
        this.changeValue(symbol, value.clamp(0, max));
    }
};
//#endregion
...
//#region  Colors
    ColorManager.outlineColor = function() {
        return OZ.core.text.outlineColor;
    };
    ColorManager.itemBackColor1 = function() {
        return OZ.core.text.itemBackColor1;
    };
    ColorManager.itemBackColor2 = function() {
        return OZ.core.text.itemBackColor2;
    };
//#endregion

//#region Rebinds
    // Menu bind change to C.
    if (OZ.core.overrideKeyInputBind) {
        Input.keyMapper = {};
        for (const bind of OZ.core.keybinds) {
            Input.keyMapper[bind.Key] = bind.Name;
        }
        Input.gamepadMapper = {};
        for (const bind of OZ.core.gamepadBinds) {
            Input.gamepadMapper[bind.Button] = bind.Name;
        }
    }
//#endregion
...
//#region Meta addons
    // Extended meta
    OZ_Metal_DataManager_extractMetadata = DataManager.extractMetadata;
    DataManager.extractMetadata = function(data) {
        OZ_Metal_DataManager_extractMetadata.call(this,data);
        const regExp2 = /<([^<>]+)>([\s\S]+?)<\/\1>/g;
        for (;;) {
            const match = regExp2.exec(data.note);
            if (match) {
...
//#region Message adjust
    // Message window margin (for big screens).
    Scene_Message.prototype.messageWindowRect = function() {
        const ww = Graphics.boxWidth - (OZ.core.message.marginHorizontal * 2);
    ...
    Window_Message.prototype.updatePlacement = function() {
        ...
        var boxHeightAdjusted = Graphics.boxHeight - (OZ.core.message.marginVertical * 2);
        this.y = (this._positionType * (boxHeightAdjusted - this.height)) / 2;
        if (goldWindow) {
            goldWindow.y = this.y > 0 ? 0 : boxHeightAdjusted - goldWindow.height;
        }
    };
//#endregion
